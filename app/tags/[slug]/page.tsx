...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!tag) {
    return getMetadata({ title: 'Tag Not Found', path: `/tags/${slug}` });
  }

  return getMetadata({
    title: `#${tag.name} Guides`,
    description: tag.description || `Browse developer guides, code samples, and articles tagged under #${tag.name}.`,
    path: `/tags/${slug}`,
    tags: [tag.name]
  });
}
...
export default async function TagDetailPage({ params }: PageProps) {
  ...
  const breadcrumbSchema = SchemaMarkup.breadcrumb([
    ...
    { name: `#${tag.name}`, url: `${baseUrl}/tags/${slug}` }
  ]);
