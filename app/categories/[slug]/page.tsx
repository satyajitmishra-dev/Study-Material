...
import { Layers, ChevronRight, Eye, ThumbsUp } from 'lucide-react';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const category = await publicDb.getCategoryBySlug(slug);
  if (!category) {
    return getMetadata({ title: 'Category Not Found', path: `/categories/${slug}` });
  }

  return getMetadata({
    title: `${category.name} Tutorials`,
    description: category.description || `Read detailed developer guides, code samples, and tutorials about ${category.name}.`,
...
export default async function CategoryDetailPage({ params }: PageProps) {
  ...
  const { items: posts } = await publicDb.getPublicPosts({ categorySlug: slug });

  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://studymaterial.utool.in';

  const collectionSchema = SchemaMarkup.collection({
    title: category.name,
    description: category.description || '',
    url: `${baseUrl}/categories/${slug}`,
    items: posts.map(p => ({ name: p.title, url: `${baseUrl}/posts/${p.slug}` }))
  });
    ...
    <>
      ...
      <div className="w-full max-w-4xl mx-auto px-4 py-12 space-y-8">
        ...
        <div className="border-b border-white/5 pb-6">
          <span className="text-[11px] font-semibold text-accent-cyan tracking-[0.2em] uppercase font-mono">Topic Archives</span>
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-warm-white mt-1">
            Category: {category.name}
          </h1>
          <p className="text-[13px] text-stone font-light mt-1.5 leading-relaxed max-w-xl">
            {category.description || 'Collection of structured tutorials, compiler studies, and reference sheets.'}
          </p>
        </div>
        ...
        <div className="space-y-4">
              ...
              <div className="flex-1 flex flex-col justify-between space-y-3">
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between text-[10px] font-mono text-stone">
                    <span className="text-accent-cyan uppercase font-semibold">{category.name}</span>
          ...
          {posts.length === 0 && (
            <div className="py-20 flex flex-col items-center justify-center border border-dashed border-white/5 rounded-2xl">
              <Layers className="w-12 h-12 text-stone/40 animate-pulse" />
              <h4 className="text-[13px] font-bold text-warm-white mt-3">No articles in category</h4>
              <p className="text-[11px] text-stone mt-1">Check back later for newly approved content.</p>
            </div>
          )}
        </div>
      </div>
