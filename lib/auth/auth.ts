...
import { getPrisma, db } from '../database/dbClient';
import { verifyPassword } from './password';
import { checkRoleRateLimit } from './rateLimiter';
import { isDisposableEmail, detectSuspiciousLogin, validateAuthConfig } from './security';
import { registerSession } from './session';
import { handleOAuthSignIn } from './oauth';
import { logAuditEvent } from './audit';
import { enqueueEmail } from './emailQueue';
import { getSuspiciousLoginHtml } from './mailer';
...
providers.push(
  Credentials({
    ...
    async authorize(credentials) {
      ...
      // CSRF / headers detection for rate limiter key
      const reqHeaders = await headers();
      const ip = reqHeaders.get('x-forwarded-for')?.split(',')[0] || reqHeaders.get('x-real-ip') || '127.0.0.1';
      const ua = reqHeaders.get('user-agent') || '';

      // 1. Rate Limit Checks
      const ipRl = await checkRoleRateLimit(`login-ip:${ip}`, 'login');
      const emailRl = await checkRoleRateLimit(`login-email:${email}`, 'login');
      ...
      // Enumeration Protection: Return null for wrong credentials to keep it generic
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    ...
    async signIn({ user, account }) {
      ...
      // Link third-party OAuth provider
      const oAuthResult = await handleOAuthSignIn({ user, account });
      if (!oAuthResult) return false;
    ...
    async jwt({ token, user, trigger }) {
      ...
      if (user && user.email) {
        ...
        if (dbUser) {
          ...
          token.emailVerified = dbUser.emailVerified ? (dbUser.emailVerified instanceof Date ? dbUser.emailVerified.toISOString() : new Date(dbUser.emailVerified).toISOString()) : null;
          ...
          if (prisma) {
            try {
              ...
              if (!exists) {
                // 1. Suspicious Login Check
                const susp = await detectSuspiciousLogin(dbUser.id, ip, ua, browser, os, 'Remote');
                if (susp.suspicious) {
                  await logAuditEvent({
                    userId: dbUser.id,
                    email: dbUser.email!,
                  ...
                  // Queue alert email asynchronously
                  await enqueueEmail(
                    dbUser.email!,
                    'Security Warning: Suspicious Login Detected - StudyMaterial',
                    getSuspiciousLoginHtml(dbUser.name || 'Developer', ip, browser, os, 'Remote Device'),
                    `Suspicious login activity caught on your account: IP ${ip}, Device ${os}`
                  );
                }
        ...
        // Handle session rotation or updates
    ...
    async session({ session, token }: any) {
      if (token.uid) {
        ...
        session.sessionToken = token.jti || null;
      }
      return session;
    }
  }
...
// Run fail-fast configurations check
validateAuthConfig();
