...
export async function sendEmail({ to, subject, html, text }: SendEmailParams) {
  ...
  if (transporter) {
    try {
      ...
      console.error('[Mailer] SMTP delivery failed, falling back to console log:', error);
    }
  }
...
export function getPasswordResetHtml(name: string, url: string) {
  const content = `
    ...
    <p>Or copy and paste this URL into your browser:</p>
    <p style="font-family: monospace; word-break: break-all; font-size: 12px; background-color: #141417; padding: 10px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.03);">${url}</p>
    <p>This link is active for <span class="highlight">1 hour</span> and can only be used once.</p>
...
export function getAccountSuspendedHtml(name: string, reason: string = 'Violation of platform community guidelines') {
  const content = `
    <h2>Account Suspension Notification</h2>
    <p>Hi ${name},</p>
    <p>Your account has been suspended by a platform administrator. During this suspension, you will be unable to log in or publish content.</p>
    <p>Reason: <span style="color: #FF5A5F; font-weight: bold;">${reason}</span></p>
    <p>If you believe this is a mistake or wish to appeal the decision, please reply directly to this email or contact support.</p>
  `;
  return getLayout('Account Suspension Notification', content);
}
